 ...
 */
@Component
public class CategoryUserPoolStrategy implements IUserPoolStrategy{

    private static final Logger LOGGER = LoggerFactory.getLogger(CategoryUserPoolStrategy.class);


    private CategoryUserPoolParam param;

    private CategoryUserPoolStrategy(){}
    public CategoryUserPoolStrategy(CategoryUserPoolParam param){
     ...
     * @param category
     */
    private void getCanFetchAndOnLineUsers(String category){
        LOGGER.info("按售后审核技能加载用户池开始:"+category);
        List<UwpAfsUserInfoExport> afsUsers = afsUserInfoRpc.getJINGDONGUserList(Integer.valueOf(category), Integer.valueOf(1));
        ...
        for(UwpAfsUserInfoExport u : afsUsers){
            erpList.add(u.getErpCode());
        }
        List<UwpUser> onLineUsers = uwpUserRpc.ifOnLineUsers(erpList, Lists.newArrayList(Integer.valueOf(1),Integer.valueOf(3)));
        ...
        LOGGER.info("按售后审核技能加载用户池完毕:" + category + "。加载人数：" +allUsers.size());
    }
    ...
    public List<UwpUserDomain> init() throws PoolLackException {
        ...
        getCanFetchAndOnLineUsers(param.getTaskAuthority());
